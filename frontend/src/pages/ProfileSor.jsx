...
export default function ProfileSor() {
  ...
  const { content, loading, refetch } = useSiteContent('profile_sor');
  ...
  const openEdit = () => {
    setForm({
      name: content.name || t('profile_sor.name'),
      tagline: content.tagline || t('profile_sor.tagline'),
      bio: content.bio || t('profile_sor.bio'),
    });
    setImageFile(null);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.put('/api/content/profile_sor', fd);
      await refetch();
      setEditing(false);
    } catch (err) {
  ...
  const displayImage = content.image_url || sorLogo;
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden">
      ...
      <div
        ...
        style={{ background: 'radial-gradient(ellipse 60% 50% at 50% 0%, rgba(212,175,55,0.07) 0%, transparent 100%)' }}
      />
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="glass-strong rounded-3xl p-8 md:p-12 text-center mb-6 animate-hero-entrance">
          ...
          <h1 className="font-serif text-3xl md:text-4xl text-cream mb-1">
            {content.name || t('profile_sor.name')}
          </h1>
          ...
          <p className="text-cream/70 text-sm italic leading-relaxed">
            {content.tagline || t('profile_sor.tagline')}
          </p>
        </div>
      ...
      {editing && (
        ...
        >
          ...
          >
            <h3 className="font-serif text-xl text-gold mb-5">แก้ไขโปรไฟล์ — ส้อ</h3>
