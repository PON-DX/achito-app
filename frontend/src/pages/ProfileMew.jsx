...
export default function ProfileMew() {
  const { t } = useLang();
  const { isAdmin } = useAuth();
  const { content, refetch } = useSiteContent('profile_mew');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.put('/api/content/profile_mew', fd);
      await refetch();
      setEditing(false);
    } catch (err) {
  ...
  const displayImage = content.image_url || mewLogo;

  return (
    <div className="min-h-screen relative overflow-x-hidden">
      <div className="fixed inset-0 -z-10 bg-gradient-to-br from-[#0d0a0b] via-[#1a1210] to-[#080603]" />
      <div
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div ref={contactRef} className="glass-strong rounded-2xl p-8 text-center" style={REVEAL(contactVisible, 0.05)}>
          ...
          <a
            href="https://www.facebook.com/ceim.sakdi.nwl.seux"
            target="_blank"
      ...
      {editing && (
        ...
        >
          ...
          >
            <h3 className="font-serif text-xl text-gold mb-5">แก้ไขโปรไฟล์ — มิว</h3>
